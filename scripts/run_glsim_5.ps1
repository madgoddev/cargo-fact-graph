...
$Bootstrap = Join-Path $PSScriptRoot "glsim_windows_bootstrap.py"
...
if ($PortOccupied) {
    throw "Port $Port is already occupied; refusing to test against an unidentified existing process."
}

$Process = Start-Process -FilePath $Python `
    -ArgumentList @($Bootstrap, "--port", "$Port", "--validators", "5") `
